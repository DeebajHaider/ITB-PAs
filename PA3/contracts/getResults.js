...
async function getResults()
{
    ...
    const sellers    = result[0];   // matchedSellers
    const buyers     = result[1];   // matchedBuyers
    ...
    for (let i = 0; i < sellers.length; i++) {
        console.log((i + 1) + '\t ' + sellers[i] + '\t ' + buyers[i] + '\t ' + price + '\t ' + quantities[i]);
    }
}
...
{
    var myAccount = "";
    await web3.eth.getAccounts().then(e => myAccount = e[0]); 
